...
class FirebaseMusicSource @Inject constructor(private val musicDatabase: MusicDatabase) {
   ...
   suspend fun fetchMediaData() = withContext(Dispatchers.IO) {
      ...
      songs = allSongs.map {
         MediaMetadataCompat.Builder()
            ...
            .putString(METADATA_KEY_DISPLAY_SUBTITLE, it.singer)
   ...
   fun asMediaItems() = songs.map {
      val descriptionSong = MediaDescriptionCompat.Builder()
         ...
         .build()
      MediaBrowserCompat.MediaItem(descriptionSong, FLAG_PLAYABLE)
   }.toMutableList()
